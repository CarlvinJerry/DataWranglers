...
comparison <- df %>% transmute("KUCCPS commitment to execute the following Corporate Social Initiatives (CSI) and activities" =  gsub("\\[|\\]", "",stringr::str_extract(df$variable,"\\[.*?\\]")))
...
freqs <- dplyr::bind_rows(transpose(df1, keep.names = "Freq", make.names = "Var1"),
                          ...
                          transpose(df7, keep.names = "Freq", make.names = "Var1")
                          # transpose(df8, keep.names = "Freq", make.names = "Var1"),
                          # transpose(df9, keep.names = "Freq", make.names = "Var1")
)
...
write.csv(final,"3csiActivities.csv")
